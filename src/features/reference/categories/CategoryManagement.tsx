...
const CategoryManagement = () => {
    ...
    return (
      <div className="flex justify-center items-center h-64">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-muted-foreground">{t('category.loading')}</p>
        </div>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        <div className="text-red-500 text-xl">{t('category.error')}</div>
  ...
  const filterConfig = [
    {
      key: 'status',
      label: t('category.filters.status'),
      options: [
        { value: 'Active', label: t('category.status.active') },
        { value: 'Inactive', label: t('category.status.inactive') }
      ]
    },
    {
      key: 'work_request_approved',
      label: t('category.filters.workRequest'),
      options: [
        { value: 'Create work order', label: t('category.workRequestOptions.createWorkOrder') },
        { value: 'Close work request', label: t('category.workRequestOptions.closeWorkRequest') }
      ]
    },
  ];
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50">
                <TableHead className="font-medium text-gray-600">{t('category.columns.code')}</TableHead>
                <TableHead className="font-medium text-gray-600">{t('category.columns.title')}</TableHead>
                <TableHead className="font-medium text-gray-600">{t('category.columns.workRequestApproved')}</TableHead>
                <TableHead className="font-medium text-gray-600">{t('category.columns.status')}</TableHead>
                <TableHead className="font-medium text-gray-600 text-right">{t('category.columns.actions')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
