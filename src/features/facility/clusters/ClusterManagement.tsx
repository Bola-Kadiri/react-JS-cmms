    ...
    return (
      <div className="flex justify-center items-center h-64">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-muted-foreground">{t('cluster.loading')}</p>
        </div>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        <div className="text-red-500 text-xl">{t('cluster.error')}</div>
  ...
  return (
    <div className="py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{t('cluster.management')}</h1>
        <Button onClick={handleAddCluster}>
          <Plus className="mr-2 h-4 w-4" /> {t('cluster.add')}
        </Button>
      </div>
            ...
            <TableHeader>
              <TableRow className="bg-gray-50">
                <TableHead className="font-medium text-gray-600">
                  ...
                  >
                    {t('cluster.columns.name')} {renderSortIcon('name')}
                  </Button>
                </TableHead>
                <TableHead className="font-medium text-gray-600">
                  <Button
                    ...
                    className="h-auto p-0 font-medium text-gray-600 hover:text-gray-900"
                  >
                    {t('cluster.columns.region')} {renderSortIcon('region')}
                  </Button>
                </TableHead>
                <TableHead className="font-medium text-gray-600">
                  <Button
                    ...
                    className="h-auto p-0 font-medium text-gray-600 hover:text-gray-900"
                  >
                    {t('cluster.columns.manager')} {renderSortIcon('manager')}
                  </Button>
                </TableHead>
                <TableHead className="font-medium text-gray-600 text-right">{t('cluster.columns.actions')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((cluster) => (
                  <TableRow key={cluster.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <TableCell>{cluster.region_detail?.name || t('cluster.unknownRegion')}</TableCell>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('cluster.delete.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('cluster.delete.message')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('cluster.delete.cancel')}</AlertDialogCancel>
            <AlertDialogAction
            ...
            >
              {deleteClusterMutation.isPending ? (
                <>
                  ...
                  {t('cluster.delete.deleting')}
                </>
              ) : (
                t('cluster.delete.confirm')
              )}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
